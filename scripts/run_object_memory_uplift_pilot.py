...
from pov_compiler.bench.reporting.object_memory_uplift import (
    compute_variant_object_memory_metrics,
    write_object_memory_compare_outputs,
)
...
def _load_yaml(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError("PyYAML is required for object-memory manifests.") from exc
...
def _run_fixture_variant(
    *,
    ...
    metrics = compute_variant_object_memory_metrics(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a small object-memory uplift pilot for current vs improved object_memory_v0.")
    parser.add_argument("--manifest", required=True, help="Object-memory uplift manifest YAML")
...
def main() -> int:
    ...
    for key in ("baseline", "uplift"):
        ...
        if source_mode == "fixture":
            ...
            raise ValueError(f"Unsupported object-memory variant source: {source_mode}")

    provider_probe = _provider_probe_note()
    outputs = write_object_memory_compare_outputs(
        out_dir=out_dir,
        suite_id=str(manifest_payload.get("suite_id", out_dir.name)),
        query_bank=query_bank,
    ...
    print(f"object_memory_logic_status={outputs['object_memory_logic_status']}")
