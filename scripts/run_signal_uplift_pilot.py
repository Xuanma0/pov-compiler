...
def _load_yaml(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError("PyYAML is required for signal uplift manifests.") from exc
...
def _provider_probe_note() -> dict[str, Any]:
    ...
    return {
        ...
        "env_present": present,
        "stdout_lines": [line for line in proc.stdout.splitlines() if "api_key_present" not in line or "true" in line or "false" in line],
    }
...
def _run_fixture_variant(
    *,
    ...
    metrics = compute_variant_signal_metrics(
        ...
        metrics_override={
            **dict(variant_cfg),
            **dict(variant_cfg.get("fixture_metrics", {})),
        },
    )
...
def _run_offline_variant(
    *,
    ...
    video_path = _resolve_path(
        str(variant_cfg.get("video", manifest_payload.get("video", ""))),
    ...
    resolved_config["perception"]["max_frames"] = int(
        variant_cfg.get("perception_max_frames", resolved_config["perception"].get("max_frames", 32))
    )
    ...
    cmd = [
        ...
        str(variant_cfg.get("perception_max_frames", resolved_config["perception"].get("max_frames", 32))),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a small signal-uplift pilot for baseline vs local YOLO26n.")
    parser.add_argument("--manifest", required=True, help="Signal uplift manifest YAML")
...
def main() -> int:
    ...
    for key in ("baseline", "uplift"):
        ...
        if source_mode == "fixture":
            ...
            raise ValueError(f"Unsupported signal uplift variant source: {source_mode}")

    provider_probe = _provider_probe_note()
    outputs = write_signal_uplift_compare_outputs(
        out_dir=out_dir,
        suite_id=str(manifest_payload.get("suite_id", out_dir.name)),
        query_bank=query_bank,
    ...
    print(f"signal_uplift_status={outputs['signal_uplift_status']}")
