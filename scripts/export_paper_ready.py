...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Optional harder sample/coverage/freeze contract output directory",
    )
    parser.add_argument(
        "--mainline-admission-closure-dir",
        default=None,
        help="Optional final mainline admission closure output directory",
    )
...
def main() -> int:
    ...
    resolved_object_persistence_uplift_dir: Path | None = None
    if args.object_persistence_uplift_dir:
        resolved_object_persistence_uplift_dir = Path(args.object_persistence_uplift_dir)
    elif args.suite_dir:
        candidate = Path(args.suite_dir) / "object_persistence_uplift"
        if candidate.exists():
            resolved_object_persistence_uplift_dir = candidate
    object_persistence_uplift_panel: dict[str, Any] = {
        "enabled": False,
        "source_dir": None,
        "copied_files": [],
    }
    object_persistence_uplift_snapshot: dict[str, Any] = {}
    if resolved_object_persistence_uplift_dir:
        object_persistence_uplift_panel["enabled"] = True
        object_persistence_uplift_panel["source_dir"] = str(resolved_object_persistence_uplift_dir)
        dst_root = out_dir / "object_persistence_uplift"
        copied = []
        for src in (
            resolved_object_persistence_uplift_dir / "tables" / "table_object_persistence_uplift_summary.csv",
            resolved_object_persistence_uplift_dir / "tables" / "table_object_persistence_uplift_summary.md",
            resolved_object_persistence_uplift_dir / "figures" / "fig_object_persistence_uplift_summary.png",
            resolved_object_persistence_uplift_dir / "figures" / "fig_object_persistence_uplift_summary.pdf",
            resolved_object_persistence_uplift_dir / "report.md",
            resolved_object_persistence_uplift_dir / "snapshot.json",
        ):
            if src.suffix.lower() in {".png", ".pdf"}:
                for dst in (dst_root / "figures" / src.name, figures_dir / src.name):
                    cp = _copy_if_exists(src, dst)
                ...
                if cp:
                    copied.append(cp)
        for src in (
            compare_dir / "tables" / "table_object_persistence_uplift.csv",
            compare_dir / "tables" / "table_object_persistence_uplift.md",
            compare_dir / "figures" / "fig_object_persistence_uplift_delta.png",
            compare_dir / "figures" / "fig_object_persistence_uplift_delta.pdf",
            compare_dir / "figures" / "fig_object_persistence_object_memory.png",
            compare_dir / "figures" / "fig_object_persistence_object_memory.pdf",
            compare_dir / "figures" / "fig_object_persistence_chain_support.png",
            compare_dir / "figures" / "fig_object_persistence_chain_support.pdf",
            compare_dir / "compare_summary.json",
            compare_dir / "snapshot.json",
        ):
            if src.suffix.lower() in {".png", ".pdf"}:
                ...
                if cp:
                    copied.append(cp)
        object_persistence_uplift_panel["copied_files"] = copied
        snapshot_src = resolved_object_persistence_uplift_dir / "snapshot.json"
        if snapshot_src.exists():
            try:
                object_persistence_uplift_snapshot = json.loads(snapshot_src.read_text(encoding="utf-8"))
            except Exception:
    ...
    resolved_harder_sample_contract_dir: Path | None = None
    if args.harder_sample_contract_dir:
        resolved_harder_sample_contract_dir = Path(args.harder_sample_contract_dir)
    harder_sample_contract_panel: dict[str, Any] = {
        "enabled": False,
        "source_dir": None,
        "copied_files": [],
    }
    harder_sample_contract_summary: dict[str, Any] = {}
    if resolved_harder_sample_contract_dir:
        harder_sample_contract_panel["enabled"] = True
        harder_sample_contract_panel["source_dir"] = str(resolved_harder_sample_contract_dir)
        dst_root = out_dir / "harder_sample_contract"
        copied = []
        for src in (
            resolved_harder_sample_contract_dir / "tables" / "table_harder_sample_contract_summary.csv",
            resolved_harder_sample_contract_dir / "tables" / "table_harder_sample_contract_summary.md",
            resolved_harder_sample_contract_dir / "report.md",
            resolved_harder_sample_contract_dir / "snapshot.json",
        ):
            if src.suffix.lower() in {".csv", ".md"} and src.parent.name == "tables":
                cp = _copy_if_exists(src, dst_root / "tables" / src.name)
                if cp:
            ...
            if cp:
                copied.append(cp)
        harder_sample_contract_panel["copied_files"] = copied
        snapshot_src = resolved_harder_sample_contract_dir / "snapshot.json"
        if snapshot_src.exists():
            try:
                harder_payload = json.loads(snapshot_src.read_text(encoding="utf-8"))
            except Exception:
                harder_payload = {}
            summary_payload = harder_payload.get("harder_sample_contract_summary", {})
            harder_sample_contract_summary = dict(summary_payload) if isinstance(summary_payload, dict) else {}

    resolved_mainline_admission_closure_dir: Path | None = None
    if args.mainline_admission_closure_dir:
        resolved_mainline_admission_closure_dir = Path(args.mainline_admission_closure_dir)
    mainline_admission_closure_panel: dict[str, Any] = {
        "enabled": False,
        "source_dir": None,
        "copied_files": [],
    }
    mainline_admission_closure_summary: dict[str, Any] = {}
    if resolved_mainline_admission_closure_dir:
        mainline_admission_closure_panel["enabled"] = True
        mainline_admission_closure_panel["source_dir"] = str(resolved_mainline_admission_closure_dir)
        dst_root = out_dir / "mainline_admission_closure"
        copied = []
        for src in (
            resolved_mainline_admission_closure_dir / "tables" / "table_mainline_admission_closure.csv",
            resolved_mainline_admission_closure_dir / "tables" / "table_mainline_admission_closure.md",
            resolved_mainline_admission_closure_dir / "report.md",
            resolved_mainline_admission_closure_dir / "snapshot.json",
        ):
            if src.suffix.lower() in {".csv", ".md"} and src.parent.name == "tables":
                cp = _copy_if_exists(src, dst_root / "tables" / src.name)
                if cp:
            ...
            if cp:
                copied.append(cp)
        mainline_admission_closure_panel["copied_files"] = copied
        snapshot_src = resolved_mainline_admission_closure_dir / "snapshot.json"
        if snapshot_src.exists():
            try:
                closure_payload = json.loads(snapshot_src.read_text(encoding="utf-8"))
            except Exception:
                closure_payload = {}
            summary_payload = closure_payload.get("mainline_admission_closure_summary", {})
            mainline_admission_closure_summary = dict(summary_payload) if isinstance(summary_payload, dict) else {}
    ...
    if resolved_object_persistence_uplift_dir:
        if object_persistence_uplift_panel.get("copied_files"):
            report_lines.extend(
                [
                    "## Object Persistence Uplift",
                    "",
                    f"- object_persistence_uplift_dir: `{object_persistence_uplift_panel.get('source_dir')}`",
                    f"- object_persistence_uplift_files: `{object_persistence_uplift_panel.get('copied_files')}`",
                    f"- object_persistence_status: `{object_persistence_uplift_snapshot.get('object_persistence_status', 'no_change')}`",
                    f"- object_memory_improved: `{object_persistence_uplift_snapshot.get('object_memory_improved', False)}`",
                    f"- object_persistence_improved: `{object_persistence_uplift_snapshot.get('object_persistence_improved', False)}`",
                    f"- lost_object_support_improved: `{object_persistence_uplift_snapshot.get('lost_object_support_improved', False)}`",
                    f"- chain_object_grounding_improved: `{object_persistence_uplift_snapshot.get('chain_object_grounding_improved', False)}`",
                    f"- query_strength_improved: `{object_persistence_uplift_snapshot.get('query_strength_improved', False)}`",
                    f"- object_persistence_recommendation: `{object_persistence_uplift_snapshot.get('next_action_recommendation', '')}`",
                    f"- should_formalize_sam3_next: `{object_persistence_uplift_snapshot.get('should_formalize_sam3_next', False)}`",
                ]
            )
        else:
    ...
    if resolved_harder_sample_contract_dir:
        if harder_sample_contract_panel.get("copied_files"):
            report_lines.extend(
                [
                    "## Harder Sample Contract",
                    "",
                    f"- harder_sample_contract_dir: `{harder_sample_contract_panel.get('source_dir')}`",
                    f"- harder_sample_contract_files: `{harder_sample_contract_panel.get('copied_files')}`",
                    f"- sample_contract_status: `{harder_sample_contract_summary.get('sample_contract_status', '')}`",
                    f"- large_sample_claim_status: `{harder_sample_contract_summary.get('large_sample_claim_status', '')}`",
                    f"- wording_recommendation: `{harder_sample_contract_summary.get('wording_recommendation', '')}`",
                ]
            )
        else:
            report_lines.append("- harder_sample_contract: source provided but artifacts missing.")
    if resolved_mainline_admission_closure_dir:
        if mainline_admission_closure_panel.get("copied_files"):
            report_lines.extend(
                [
                    "## Mainline Admission Closure",
                    "",
                    f"- mainline_admission_closure_dir: `{mainline_admission_closure_panel.get('source_dir')}`",
                    f"- mainline_admission_closure_files: `{mainline_admission_closure_panel.get('copied_files')}`",
                    f"- mainline_admission_closure_status: `{mainline_admission_closure_summary.get('mainline_admission_closure_status', '')}`",
                    f"- promotion_ready: `{mainline_admission_closure_summary.get('promotion_ready', False)}`",
                    f"- mainline_admission_ready: `{mainline_admission_closure_summary.get('mainline_admission_ready', False)}`",
                    f"- recommended_wording: `{mainline_admission_closure_summary.get('recommended_wording', '')}`",
                    f"- recommended_next_step: `{mainline_admission_closure_summary.get('recommended_next_step', '')}`",
                ]
            )
        else:
            report_lines.append("- mainline_admission_closure: source provided but artifacts missing.")
    if (
        ...
        or resolved_harder_sample_contract_dir
        or resolved_mainline_admission_closure_dir
    ):
        report_lines.extend(
            [
                ...
                "- Treat `sample_contract/` as the v1.59 baseline contract panel after the harder contract.",
            ]
        )
    if resolved_query_strength_audit_dir:
    ...
    snapshot = {
        ...
        "inputs": {
            ...
            "harder_sample_contract_dir": str(resolved_harder_sample_contract_dir)
            if resolved_harder_sample_contract_dir
            else None,
            "mainline_admission_closure_dir": str(resolved_mainline_admission_closure_dir)
            if resolved_mainline_admission_closure_dir
            else None,
        ...
        "outputs": {
            ...
            "harder_sample_contract_panel": harder_sample_contract_panel,
            "mainline_admission_closure_panel": mainline_admission_closure_panel,
    ...
    if bool(args.export_submission_pack):
        ...
        if resolved_harder_sample_contract_dir:
            cmd.extend(["--harder-sample-contract-dir", str(resolved_harder_sample_contract_dir)])
        if resolved_mainline_admission_closure_dir:
            cmd.extend(["--mainline-admission-closure-dir", str(resolved_mainline_admission_closure_dir)])
        if resolved_query_strength_audit_dir:
