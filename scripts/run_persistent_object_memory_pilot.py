...
from pov_compiler.bench.reporting.persistent_object_memory_uplift import (
    compute_variant_persistent_object_memory_metrics,
    write_persistent_object_memory_compare_outputs,
)
...
def _query_bank_lock_payload(*, manifest_payload: dict[str, Any], manifest_path: Path, query_bank: QueryBank) -> dict[str, Any]:
    query_bank_path = _resolve_path(str(manifest_payload.get("query_bank", "")).strip(), manifest_path.parent)
...
def _run_fixture_variant(*, variant_label: str, variant_cfg: dict[str, Any], query_bank: QueryBank, out_dir: Path) -> dict[str, Any]:
    metrics = compute_variant_persistent_object_memory_metrics(
...
def _run_offline_variant(
    *,
    ...
    variant_memory_cfg = variant_cfg.get("object_memory", {})
    if isinstance(variant_memory_cfg, dict):
        resolved_config["object_memory"].update(json.loads(json.dumps(variant_memory_cfg)))
    ...
    metrics = compute_variant_persistent_object_memory_metrics(
        ...
        source_mode="offline",
        metrics_override={
            "perception_backend": str(variant_cfg.get("perception_backend", "")),
            "object_memory_logic_variant": str(resolved_config["object_memory"].get("logic_variant", "")),
        },
    )
    _write_json(out_dir / "metrics.json", metrics)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a persistent object-memory uplift pilot for persistence_v1 vs persistent_v2.")
    parser.add_argument("--manifest", required=True, help="Persistent object-memory uplift manifest YAML")
...
def main() -> int:
    ...
    query_bank_lock_payload = _query_bank_lock_payload(manifest_payload=manifest_payload, manifest_path=manifest_path, query_bank=query_bank)
    ...
    for key in ("baseline", "uplift"):
        ...
        if source_mode == "fixture":
            variant_results[key] = _run_fixture_variant(variant_label=variant_label, variant_cfg=variant_cfg, query_bank=query_bank, out_dir=variant_dir)
            ...
            raise ValueError(f"Unsupported persistent object-memory variant source: {source_mode}")

    provider_probe = _provider_probe_note()
    outputs = write_persistent_object_memory_compare_outputs(
        out_dir=out_dir,
        suite_id=str(manifest_payload.get("suite_id", out_dir.name)),
        query_bank=query_bank,
    ...
    print(f"persistent_object_memory_status={outputs['persistent_object_memory_status']}")
