...
def test_submission_pack_signal_uplift_smoke(tmp_path: Path) -> None:
    ...
    (paper_ready_dir / "signal_uplift" / "report.md").write_text("# uplift report\n", encoding="utf-8")
    (paper_ready_dir / "signal_uplift" / "snapshot.json").write_text(
        json.dumps({"signal_uplift_status": "improved", "should_try_sam3_next": False}),
        encoding="utf-8",
    )
    ...
    (signal_uplift_dir / "tables" / "table_signal_uplift_summary.csv").write_text("signal_uplift_status\nimproved\n", encoding="utf-8")
    (signal_uplift_dir / "report.md").write_text("# signal uplift report\n", encoding="utf-8")
    (signal_uplift_dir / "snapshot.json").write_text(
        json.dumps({"signal_uplift_status": "improved", "next_action_recommendation": "keep_yolo26n_and_scale_real"}),
        encoding="utf-8",
    )
    ...
    assert "perception-limited" in readme_text
