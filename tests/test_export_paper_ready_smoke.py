...
def test_export_paper_ready_smoke(tmp_path: Path) -> None:
    ...
    object_persistence_uplift_dir = tmp_path / "object_persistence_uplift"
    (object_persistence_uplift_dir / "tables").mkdir(parents=True, exist_ok=True)
    (object_persistence_uplift_dir / "figures").mkdir(parents=True, exist_ok=True)
    (object_persistence_uplift_dir / "tables" / "table_object_persistence_uplift_summary.csv").write_text(
        "suite_id,object_persistence_status,object_memory_improved,object_persistence_improved,lost_object_support_improved,chain_object_grounding_improved,query_strength_improved,next_action_recommendation,should_formalize_sam3_next\nv155_signal_uplift_real,improved,True,True,True,True,True,promote_sam3_to_next_stage,True\n",
        encoding="utf-8",
    )
    (object_persistence_uplift_dir / "tables" / "table_object_persistence_uplift_summary.md").write_text(
        "# object persistence uplift\n",
        encoding="utf-8",
    )
    (object_persistence_uplift_dir / "figures" / "fig_object_persistence_uplift_summary.png").write_bytes(b"PNG")
    (object_persistence_uplift_dir / "figures" / "fig_object_persistence_uplift_summary.pdf").write_bytes(b"PDF")
    (object_persistence_uplift_dir / "report.md").write_text("# object persistence uplift report\n", encoding="utf-8")
    (object_persistence_uplift_dir / "snapshot.json").write_text(
        json.dumps(
            {
    ...
    persistent_object_memory_uplift_dir = tmp_path / "persistent_object_memory_uplift"
    (persistent_object_memory_uplift_dir / "tables").mkdir(parents=True, exist_ok=True)
    (persistent_object_memory_uplift_dir / "figures").mkdir(parents=True, exist_ok=True)
    (persistent_object_memory_uplift_dir / "tables" / "table_persistent_object_memory_uplift_summary.csv").write_text(
        "suite_id,persistent_object_memory_status,object_memory_improved,short_term_memory_improved,long_term_memory_improved,reappearance_support_improved,lost_object_support_improved,chain_object_grounding_improved,query_strength_improved,next_action_recommendation,should_formalize_persistent_object_memory\nv157_object_memory_real,improved,True,True,True,True,True,True,True,promote_persistent_object_memory,True\n",
        encoding="utf-8",
    )
    (persistent_object_memory_uplift_dir / "tables" / "table_persistent_object_memory_uplift_summary.md").write_text(
        "# persistent object memory uplift\n",
        encoding="utf-8",
    )
    (persistent_object_memory_uplift_dir / "figures" / "fig_persistent_object_memory_uplift_summary.png").write_bytes(b"PNG")
    (persistent_object_memory_uplift_dir / "figures" / "fig_persistent_object_memory_uplift_summary.pdf").write_bytes(b"PDF")
    (persistent_object_memory_uplift_dir / "report.md").write_text("# persistent object memory uplift report\n", encoding="utf-8")
    (persistent_object_memory_uplift_dir / "snapshot.json").write_text(
        json.dumps(
            {
                "persistent_object_memory_status": "improved",
                "object_memory_improved": True,
                "short_term_memory_improved": True,
    ...
    (compare_dir / "tables" / "table_object_persistence_uplift.csv").write_text(
        "variant_label,object_memory_items_total,persistent_tracks_total,lost_object_query_support_rate,chain_object_grounding_support_rate,object_persistence_status\nbaseline,4,0,0.25,0.20,improved\nuplift,6,5,0.75,0.67,improved\ndelta,2,5,0.50,0.47,improved\n",
        encoding="utf-8",
    )
    (compare_dir / "tables" / "table_object_persistence_uplift.md").write_text(
        "# object persistence uplift compare\n",
        encoding="utf-8",
    )
    (compare_dir / "figures" / "fig_object_persistence_uplift_delta.png").write_bytes(b"PNG")
    (compare_dir / "figures" / "fig_object_persistence_uplift_delta.pdf").write_bytes(b"PDF")
    (compare_dir / "figures" / "fig_object_persistence_object_memory.png").write_bytes(b"PNG")
    ...
    (compare_dir / "tables" / "table_persistent_object_memory_uplift.csv").write_text(
        "variant_label,object_memory_items_total,object_memory_long_term_items_total,reappearance_support_rate,persistent_object_memory_status\nbaseline,7,1,0.25,improved\nuplift,8,3,0.75,improved\ndelta,1,2,0.5,improved\n",
        encoding="utf-8",
    )
    (compare_dir / "tables" / "table_persistent_object_memory_uplift.md").write_text(
        "# persistent object memory uplift compare\n",
        encoding="utf-8",
    )
    (compare_dir / "figures" / "fig_persistent_object_memory_delta.png").write_bytes(b"PNG")
    (compare_dir / "figures" / "fig_persistent_object_memory_delta.pdf").write_bytes(b"PDF")
    (compare_dir / "figures" / "fig_persistent_object_memory_lost_object.png").write_bytes(b"PNG")
    (compare_dir / "figures" / "fig_persistent_object_memory_lost_object.pdf").write_bytes(b"PDF")
    (compare_dir / "figures" / "fig_persistent_object_memory_chain_support.png").write_bytes(b"PNG")
    (compare_dir / "figures" / "fig_persistent_object_memory_chain_support.pdf").write_bytes(b"PDF")
    (compare_dir / "figures" / "fig_persistent_object_memory_reappearance.png").write_bytes(b"PNG")
    (compare_dir / "figures" / "fig_persistent_object_memory_reappearance.pdf").write_bytes(b"PDF")
    ...
    cmd = [
        ...
        str(persistent_object_memory_uplift_dir),
    ...
    assert "## Persistent Object Memory Uplift" in report_text
    ...
    assert (out_dir / "persistent_object_memory_uplift" / "tables" / "table_persistent_object_memory_uplift_summary.csv").exists()
    assert (out_dir / "persistent_object_memory_uplift" / "report.md").exists()
    ...
    assert (out_dir / "figures" / "fig_object_persistence_uplift_summary.png").exists()
    ...
    assert (out_dir / "figures" / "fig_persistent_object_memory_uplift_summary.png").exists()
    assert (out_dir / "figures" / "fig_persistent_object_memory_delta.png").exists()
    assert (out_dir / "figures" / "fig_persistent_object_memory_lost_object.png").exists()
    assert (out_dir / "figures" / "fig_persistent_object_memory_chain_support.png").exists()
